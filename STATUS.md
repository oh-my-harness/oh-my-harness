...
- 结果对比图与说明：`glm-5.3-coding-benchmarks/docs/deepswe-local-results-20260911.md`（官方 GLM-5.3 400K `0.669`、本地 GLM-5.3 256K `0.5752`、官方 Flash 400K `0.634`、本地 Flash 400K protocol `0.6549`；GLM-5.3 的 9/113 个 256K context-overflow 任务保持失败标注）。
- 2026-09-11 开始 GLM-5.3-Flash Terminal-Bench 2.1 本地对照。已确认 Hyperop Flash 可用，LiteLLM Anthropic Messages facade 返回 HTTP 200；Harbor 0.22.0 可解析本地 TB2.1 数据。首次 smoke 未进入 agent 阶段，阻塞在 `/etc/subuid`/`/etc/subgid` 缺少 `leiqiaojie` 条目导致 rootless 容器无法解镜像层；计划与状态见 `glm-5.3-coding-benchmarks/docs/terminal-bench-flash-plan.md`。
- 2026-09-11 已核对并修正 Flash 官方数据：GLM-5.3-Flash 模型上下文为 1M、最大输出 128K；官方 TB2.1 `84.3`、DeepSWE v1.1 `63.4`。已完成 Flash DeepSWE 本地 run 使用公开 benchmark 的 400K contract，不是 256K variant。
...
- **CI 修复**：[PR #9](https://github.com/oh-my-harness/senza-studio/pull/9) 已 squash 合入 `main`（merge commit `3df6ba1`）。Python job 不再依赖 `PRIVATE_REPO_TOKEN` 或私有源码 checkout，改为安装公开发布的 `senza-sdk==1.3.0` wheel；`senza-sdk.lock` 记录版本与源码 commit `320744cf4fd96399fcc3d4111c357b85c4e1545b`，安装后执行版本校验，Rust toolchain/Cargo cache/本地 wheel 构建已移除。已确认 PyPI wheel 与 GitHub Release `v1.3.0` 资产一致，fork PR secret 限制与 [issue #7](https://github.com/oh-my-harness/senza-studio/issues/7) 已解决。
...
### 2026-09-19 llm-harness-runtime App 数据迁移控制面

**仓库**：`llm-harness-runtime`；PR #214 已合并 `main`（merge commit `66bf489`，功能 commit `6fac48f`）。

- **CLI 控制面**：`llm-harness-app migrate-data --from SOURCE --to DESTINATION [--backup BACKUP]` 先完整复制 staging 并写入私有 report/journal，再执行 source→backup、staging→destination 的 durable rename；`rollback-data` 恢复原数据根，并将迁移后变更的目标目录保留到独立 sibling 目录。
- **恢复语义**：journal 覆盖 Prepared、SourceRenamed、Completed、RolledBack 与 rollback 中断点；重复执行同一命令可恢复提交、补写 rollback report 或清理 rolled-back staging，不会误续提交。
...
**仓库**：`llm-harness-runtime`；PR #217 已合并 `main`（merge commit `6ad3b6a`，功能 commit `5699b2d`，refs #193）。
...
- **状态**：PR #217 已合并；本切片只完成通用 frame transport，不包含 guest agent 实现、Firecracker lifecycle、镜像/Jailer/网络策略或部署验证。#193 继续保持 open。
...
**仓库**：`llm-harness-runtime`；PR #228 已合并 `main`（merge commit `280b514`，功能 commit `182bed7`，refs #193）；follow-up PR [#229](https://github.com/oh-my-harness/llm-harness-runtime/pull/229) 已合并 `main`（merge commit `8e4d529`，功能 commits `041da03` / `12f60f0`）。
...
**仓库**：`llm-harness-runtime`；PR [#230](https://github.com/oh-my-harness/llm-harness-runtime/pull/230) 已合并 `main`（merge commit `b66d234`，功能 commits `373073d` / `95e57ea`，refs #193）。
...
**仓库**：`llm-harness-runtime`；PR [#231](https://github.com/oh-my-harness/llm-harness-runtime/pull/231) 已合并 `main`（merge commit `3dacc76`，功能 commit `5e67bb3`，refs #193），远端功能分支已删除。
...
### 2026-09-22 llm-harness-runtime Firecracker graceful stop

**仓库**：`llm-harness-runtime`；PR [#233](https://github.com/oh-my-harness/llm-harness-runtime/pull/233) 已合并 `main`（merge commit `9d67281`，功能 commit `7b6c8a7`，refs #193），远端功能分支已删除。

- **优雅停止**：`FirecrackerProcess::graceful_stop(timeout)` 通过 Firecracker Unix API 发送 `PUT /actions` / `SendCtrlAltDel`，HTTP 响应只读取状态行且限制在 1 KiB 内，避免异常 server 造成无界读取。
- **超时兜底**：请求与等待退出均受调用方 timeout 控制；信号失败、请求超时或进程未退出时会回落到现有 force-stop 路径，等待 kill 后清理 API socket 与 config file，并返回包含主因和 fallback 结果的错误。
- **Lifecycle 集成**：`FirecrackerLifecycle::graceful_stop(timeout)` 复用 process graceful stop，成功后删除 ephemeral image；重复调用保持幂等，Drop 仍保留 force cleanup 兜底。
- **测试覆盖**：fake Unix HTTP server 验证 `SendCtrlAltDel` 请求与 204 响应、artifact 清理、幂等；无响应 server 验证请求超时后强制 kill 与清理；lifecycle 集成测试覆盖优雅停止后 rootfs/API/config 均消失。crate 测试 33/33 通过。
- **验证**：`cargo test -p llm-harness-vm-firecracker --all-targets`、`cargo clippy -p llm-harness-vm-firecracker --all-targets --all-features -- -D warnings`、`cargo fmt --check`、`git diff --check` 与 Windows target `cargo check` 通过；本切片同时修复 Windows 下 `process.rs` 既有 `Duration` cfg import warning。
- **状态**：PR #233 远端 CI 仍为 GitHub billing/spending limit 导致 job 未启动，非代码失败。#193 继续保持 open；guest-agent-coordinated shutdown confirmation、jailer、registry recovery、网络策略、gateway 端到端集成与部署验证仍未完成。

### 2026-09-22 llm-harness-runtime AgentTeam 成员移除 timeout 修复

**仓库**：`llm-harness-runtime`；PR [#235](https://github.com/oh-my-harness/llm-harness-runtime/pull/235) 已合并 `main`（merge commit `4994f7b`，功能 HEAD `ab8d365`），远端功能分支已删除。
...
**仓库**：`llm-harness-runtime`；PR [#243](https://github.com/oh-my-harness/llm-harness-runtime/pull/243) 已合并 `main`（feature head `e1db161c56618ab23d5927cd78fe888397bcd2cd`，merge commit `92a5b9c8560374813d7c4ca4d918bf2a802f8a04`，refs #207）。
...
**仓库**：`llm-harness-runtime`；PR [#247](https://github.com/oh-my-harness/llm-harness-runtime/pull/247)（feature head `c4196c78ca9bed31668b8c3bd139bdde4b3bcc5f`，merge commit `409f9961ec0afc6468fbaaf6947dfa27d93e8727`，refs [#245](https://github.com/oh-my-harness/llm-harness-runtime/issues/245)，已合并）。
