...
> 最后更新：2026-09-01（agent-team pending timer SQLite journal 已覆盖 clean-shutdown 恢复、回复取消清理、timer 写入显式失败和重启去重；agent-team 165 项与 9 项集成、studio 99 项单元与 7 项 mock 集成测试通过，fmt/clippy 通过。此前完整 11 成员 coding-team 本地 LLM E2E 已通过：220.11s，judge 收齐 5/5 reviewer 报告并独立验证 PASS。）
...
### 2026-09-01 agent-team pending timer 持久化

**仓库**：`llm-harness-runtime`，分支 `feat/agent-team-timer-journal`（PR #158）。

- **恢复语义**：新增 `TimeoutJournal`，timeout 和 self-reminder 创建前先写入 `teams/<id>/timers.sqlite3`；team rebuild 时读取 journal，到期 timer 立即投递，未来 timer 按剩余时间重建。
- **生命周期清理**：timer 成功投递 inbox 后删除 timer row；delegate/broadcast 回复在超时前取消时同步删除对应 row，避免恢复后重复触发。
- **关系事件保留**：委托 timer 恢复触发时仍会标记 fired 并产生 `FailedCollaboration`；self-reminder 仍按命名空间 key 清理，不干扰同 task 的 delegate timeout。
- **回归测试**：新增 journal CRUD、到期 timer 恢复投递、未到期 timer 回复取消三项测试。
- **验证**：`agent-team` 163/163 单元 + 9/9 集成；`agent-team-studio` 99/99 单元 + 7/7 mock 集成；`cargo fmt --all -- --check` 和 targeted clippy（`-D warnings`）通过。
- **待办**：将 timer journal 写失败从“记录日志但继续”改为向社交工具返回显式失败；为 timer 触发与 inbox 投递之间的崩溃窗口增加端到端 redelivery 去重。
