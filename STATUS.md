...
### llm-harness-runtime ✅ v0.2 已实现（2026-06-12）
设计文档位于 `llm-harness-runtime/docs/runtime-v0.2-design.md`。

8 个 crate，68 个测试全部通过：

| crate | 内容 |
|-------|------|
| `llm-harness-runtime` | 主 crate：全部 trait 定义 + TaskRunner 状态机 + BudgetControlAdapter + HumanApprovalWrapper + TracingHookAdapter + CompositeHook 系列 |
...
**待做（v0.2.1）：**
- E2E 测试（用 `MockLlmClient` 跑完整 Task 生命周期），需 llm-harness-core 的 `test-utils` feature
...
1. **runtime v0.2.1**：补全 E2E 测试（MockLlmClient 跑完整 Task 生命周期）
2. **将 coding-agent 的临时实现迁移到 runtime**（tools/settings/provider 选择逻辑）
3. **eda-agent** 设计参考文档位于 `/data/leiqiaojie2/eda-agent-design.md`（已有详细设计，待迁移完成后开发）
